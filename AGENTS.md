...
**A fonte clicável é o Cronograma de Preparação de Urnas de 18/09/2026** (PDF público na página Eleições 2026 do TRE-PB, 1º e 2º turnos), o mesmo nos 10 cards. O 1º turno desse cronograma é idêntico ao Anexo I do Edital nº 15/2026 TRE-PB/PTRE/ASPRE, conferido zona a zona; por isso a `descricao` do 1º turno **cita** o edital, sem hyperlink. O 2º turno ainda não tem edital e a descrição não cita nenhum. A cadeia foi PDF v2 → minuta → Edital 14 → Edital 15 → cronograma de 18/09. A cada nova versão, confira a tabela inteira antes de aplicar — as mudanças têm vindo em uma ou duas linhas. Há teste impedindo que qualquer dos elos anteriores volte.

**O PDF de 18/09 erra o dia da semana em três linhas** (33ª, 34ª, 52ª); as datas estão certas. `diaSemana` é calculado da data pelo gerador, nunca copiado do PDF, e há teste para isso.
